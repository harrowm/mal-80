// ============================================================================
// TRS-80 Model I Character Generator ROM
// ============================================================================
//
// The TRS-80 Model I uses a separate character generator ROM chip (MCM6670P)
// that is NOT part of the Level I or Level II BASIC ROMs.
//
// 128 characters x 8 bytes each = 1024 bytes
// Each byte = one row of a 6-wide dot matrix (bits 5..0, MSB first)
//
// Generated by extract_char_rom.py
// ============================================================================

#pragma once
#include <cstdint>

constexpr uint16_t TRS80_CHAR_COUNT = 128;
constexpr uint16_t TRS80_CHAR_BYTES_PER_CHAR = 8;
constexpr uint16_t TRS80_CHAR_GEN_SIZE = 1024;

// Character Generator ROM Data (MCM6670P compatible)
constexpr uint8_t TRS80_CHAR_GEN[TRS80_CHAR_GEN_SIZE] = {

    // ---- 0x00 - 0x0F ----
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  // 0x00 0x00
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  // 0x01 0x01
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  // 0x02 0x02
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  // 0x03 0x03
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  // 0x04 0x04
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  // 0x05 0x05
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  // 0x06 0x06
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  // 0x07 0x07
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  // 0x08 0x08
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  // 0x09 0x09
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  // 0x0A 0x0A
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  // 0x0B 0x0B
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  // 0x0C 0x0C
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  // 0x0D 0x0D
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  // 0x0E 0x0E
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  // 0x0F 0x0F

    // ---- 0x10 - 0x1F ----
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  // 0x10 0x10
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  // 0x11 0x11
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  // 0x12 0x12
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  // 0x13 0x13
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  // 0x14 0x14
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  // 0x15 0x15
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  // 0x16 0x16
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  // 0x17 0x17
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  // 0x18 0x18
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  // 0x19 0x19
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  // 0x1A 0x1A
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  // 0x1B 0x1B
    0x08, 0x1C, 0x3E, 0x08, 0x08, 0x08, 0x08, 0x00,  // 0x1C 0x1C
    0x08, 0x08, 0x08, 0x08, 0x3E, 0x1C, 0x08, 0x00,  // 0x1D 0x1D
    0x00, 0x08, 0x04, 0x3E, 0x04, 0x08, 0x00, 0x00,  // 0x1E 0x1E
    0x00, 0x08, 0x10, 0x3E, 0x10, 0x08, 0x00, 0x00,  // 0x1F 0x1F

    // ---- 0x20 - 0x2F ----
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  // 0x20  
    0x08, 0x08, 0x08, 0x08, 0x08, 0x00, 0x08, 0x00,  // 0x21 !
    0x14, 0x14, 0x14, 0x00, 0x00, 0x00, 0x00, 0x00,  // 0x22 "
    0x14, 0x14, 0x3E, 0x14, 0x3E, 0x14, 0x14, 0x00,  // 0x23 #
    0x08, 0x1E, 0x28, 0x1C, 0x0A, 0x3C, 0x08, 0x00,  // 0x24 $
    0x30, 0x32, 0x04, 0x08, 0x10, 0x26, 0x06, 0x00,  // 0x25 %
    0x10, 0x28, 0x28, 0x10, 0x2A, 0x24, 0x1A, 0x00,  // 0x26 &
    0x08, 0x08, 0x10, 0x00, 0x00, 0x00, 0x00, 0x00,  // 0x27 '
    0x04, 0x08, 0x10, 0x10, 0x10, 0x08, 0x04, 0x00,  // 0x28 (
    0x10, 0x08, 0x04, 0x04, 0x04, 0x08, 0x10, 0x00,  // 0x29 )
    0x00, 0x08, 0x2A, 0x1C, 0x2A, 0x08, 0x00, 0x00,  // 0x2A *
    0x00, 0x08, 0x08, 0x3E, 0x08, 0x08, 0x00, 0x00,  // 0x2B +
    0x00, 0x00, 0x00, 0x00, 0x00, 0x08, 0x08, 0x10,  // 0x2C ,
    0x00, 0x00, 0x00, 0x3E, 0x00, 0x00, 0x00, 0x00,  // 0x2D -
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x08, 0x00,  // 0x2E .
    0x00, 0x02, 0x04, 0x08, 0x10, 0x20, 0x00, 0x00,  // 0x2F /

    // ---- 0x30 - 0x3F ----
    0x1C, 0x22, 0x26, 0x2A, 0x32, 0x22, 0x1C, 0x00,  // 0x30 0
    0x08, 0x18, 0x08, 0x08, 0x08, 0x08, 0x1C, 0x00,  // 0x31 1
    0x1C, 0x22, 0x02, 0x0C, 0x10, 0x20, 0x3E, 0x00,  // 0x32 2
    0x3E, 0x02, 0x04, 0x0C, 0x02, 0x22, 0x1C, 0x00,  // 0x33 3
    0x04, 0x0C, 0x14, 0x24, 0x3E, 0x04, 0x04, 0x00,  // 0x34 4
    0x3E, 0x20, 0x3C, 0x02, 0x02, 0x22, 0x1C, 0x00,  // 0x35 5
    0x0C, 0x10, 0x20, 0x3C, 0x22, 0x22, 0x1C, 0x00,  // 0x36 6
    0x3E, 0x02, 0x04, 0x08, 0x10, 0x10, 0x10, 0x00,  // 0x37 7
    0x1C, 0x22, 0x22, 0x1C, 0x22, 0x22, 0x1C, 0x00,  // 0x38 8
    0x1C, 0x22, 0x22, 0x1E, 0x02, 0x04, 0x18, 0x00,  // 0x39 9
    0x00, 0x00, 0x08, 0x00, 0x00, 0x08, 0x00, 0x00,  // 0x3A :
    0x00, 0x00, 0x08, 0x00, 0x00, 0x08, 0x08, 0x10,  // 0x3B ;
    0x04, 0x08, 0x10, 0x20, 0x10, 0x08, 0x04, 0x00,  // 0x3C <
    0x00, 0x00, 0x3E, 0x00, 0x3E, 0x00, 0x00, 0x00,  // 0x3D =
    0x10, 0x08, 0x04, 0x02, 0x04, 0x08, 0x10, 0x00,  // 0x3E >
    0x1C, 0x22, 0x02, 0x04, 0x08, 0x00, 0x08, 0x00,  // 0x3F ?

    // ---- 0x40 - 0x4F ----
    0x1C, 0x22, 0x2A, 0x2E, 0x2C, 0x20, 0x1E, 0x00,  // 0x40 @
    0x08, 0x14, 0x22, 0x22, 0x3E, 0x22, 0x22, 0x00,  // 0x41 A
    0x3C, 0x22, 0x22, 0x3C, 0x22, 0x22, 0x3C, 0x00,  // 0x42 B
    0x1C, 0x22, 0x20, 0x20, 0x20, 0x22, 0x1C, 0x00,  // 0x43 C
    0x38, 0x24, 0x22, 0x22, 0x22, 0x24, 0x38, 0x00,  // 0x44 D
    0x3E, 0x20, 0x20, 0x3C, 0x20, 0x20, 0x3E, 0x00,  // 0x45 E
    0x3E, 0x20, 0x20, 0x3C, 0x20, 0x20, 0x20, 0x00,  // 0x46 F
    0x1C, 0x22, 0x20, 0x2E, 0x22, 0x22, 0x1C, 0x00,  // 0x47 G
    0x22, 0x22, 0x22, 0x3E, 0x22, 0x22, 0x22, 0x00,  // 0x48 H
    0x1C, 0x08, 0x08, 0x08, 0x08, 0x08, 0x1C, 0x00,  // 0x49 I
    0x02, 0x02, 0x02, 0x02, 0x02, 0x22, 0x1C, 0x00,  // 0x4A J
    0x22, 0x24, 0x28, 0x30, 0x28, 0x24, 0x22, 0x00,  // 0x4B K
    0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x3E, 0x00,  // 0x4C L
    0x22, 0x36, 0x2A, 0x2A, 0x22, 0x22, 0x22, 0x00,  // 0x4D M
    0x22, 0x32, 0x2A, 0x26, 0x22, 0x22, 0x22, 0x00,  // 0x4E N
    0x1C, 0x22, 0x22, 0x22, 0x22, 0x22, 0x1C, 0x00,  // 0x4F O

    // ---- 0x50 - 0x5F ----
    0x3C, 0x22, 0x22, 0x3C, 0x20, 0x20, 0x20, 0x00,  // 0x50 P
    0x1C, 0x22, 0x22, 0x22, 0x2A, 0x24, 0x1A, 0x00,  // 0x51 Q
    0x3C, 0x22, 0x22, 0x3C, 0x28, 0x24, 0x22, 0x00,  // 0x52 R
    0x1C, 0x22, 0x20, 0x1C, 0x02, 0x22, 0x1C, 0x00,  // 0x53 S
    0x3E, 0x08, 0x08, 0x08, 0x08, 0x08, 0x08, 0x00,  // 0x54 T
    0x22, 0x22, 0x22, 0x22, 0x22, 0x22, 0x1C, 0x00,  // 0x55 U
    0x22, 0x22, 0x22, 0x22, 0x14, 0x14, 0x08, 0x00,  // 0x56 V
    0x22, 0x22, 0x22, 0x2A, 0x2A, 0x36, 0x22, 0x00,  // 0x57 W
    0x22, 0x22, 0x14, 0x08, 0x14, 0x22, 0x22, 0x00,  // 0x58 X
    0x22, 0x22, 0x14, 0x08, 0x08, 0x08, 0x08, 0x00,  // 0x59 Y
    0x3E, 0x02, 0x04, 0x08, 0x10, 0x20, 0x3E, 0x00,  // 0x5A Z
    0x1C, 0x10, 0x10, 0x10, 0x10, 0x10, 0x1C, 0x00,  // 0x5B [
    0x00, 0x20, 0x10, 0x08, 0x04, 0x02, 0x00, 0x00,  // 0x5C backslash
    0x1C, 0x04, 0x04, 0x04, 0x04, 0x04, 0x1C, 0x00,  // 0x5D ]
    0x08, 0x14, 0x22, 0x00, 0x00, 0x00, 0x00, 0x00,  // 0x5E ^
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x3E, 0x00,  // 0x5F _

    // ---- 0x60 - 0x6F ----
    0x1C, 0x22, 0x2A, 0x2E, 0x2C, 0x20, 0x1E, 0x00,  // 0x60 `
    0x08, 0x14, 0x22, 0x22, 0x3E, 0x22, 0x22, 0x00,  // 0x61 a
    0x3C, 0x22, 0x22, 0x3C, 0x22, 0x22, 0x3C, 0x00,  // 0x62 b
    0x1C, 0x22, 0x20, 0x20, 0x20, 0x22, 0x1C, 0x00,  // 0x63 c
    0x38, 0x24, 0x22, 0x22, 0x22, 0x24, 0x38, 0x00,  // 0x64 d
    0x3E, 0x20, 0x20, 0x3C, 0x20, 0x20, 0x3E, 0x00,  // 0x65 e
    0x3E, 0x20, 0x20, 0x3C, 0x20, 0x20, 0x20, 0x00,  // 0x66 f
    0x1C, 0x22, 0x20, 0x2E, 0x22, 0x22, 0x1C, 0x00,  // 0x67 g
    0x22, 0x22, 0x22, 0x3E, 0x22, 0x22, 0x22, 0x00,  // 0x68 h
    0x1C, 0x08, 0x08, 0x08, 0x08, 0x08, 0x1C, 0x00,  // 0x69 i
    0x02, 0x02, 0x02, 0x02, 0x02, 0x22, 0x1C, 0x00,  // 0x6A j
    0x22, 0x24, 0x28, 0x30, 0x28, 0x24, 0x22, 0x00,  // 0x6B k
    0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x3E, 0x00,  // 0x6C l
    0x22, 0x36, 0x2A, 0x2A, 0x22, 0x22, 0x22, 0x00,  // 0x6D m
    0x22, 0x32, 0x2A, 0x26, 0x22, 0x22, 0x22, 0x00,  // 0x6E n
    0x1C, 0x22, 0x22, 0x22, 0x22, 0x22, 0x1C, 0x00,  // 0x6F o

    // ---- 0x70 - 0x7F ----
    0x3C, 0x22, 0x22, 0x3C, 0x20, 0x20, 0x20, 0x00,  // 0x70 p
    0x1C, 0x22, 0x22, 0x22, 0x2A, 0x24, 0x1A, 0x00,  // 0x71 q
    0x3C, 0x22, 0x22, 0x3C, 0x28, 0x24, 0x22, 0x00,  // 0x72 r
    0x1C, 0x22, 0x20, 0x1C, 0x02, 0x22, 0x1C, 0x00,  // 0x73 s
    0x3E, 0x08, 0x08, 0x08, 0x08, 0x08, 0x08, 0x00,  // 0x74 t
    0x22, 0x22, 0x22, 0x22, 0x22, 0x22, 0x1C, 0x00,  // 0x75 u
    0x22, 0x22, 0x22, 0x22, 0x14, 0x14, 0x08, 0x00,  // 0x76 v
    0x22, 0x22, 0x22, 0x2A, 0x2A, 0x36, 0x22, 0x00,  // 0x77 w
    0x22, 0x22, 0x14, 0x08, 0x14, 0x22, 0x22, 0x00,  // 0x78 x
    0x22, 0x22, 0x14, 0x08, 0x08, 0x08, 0x08, 0x00,  // 0x79 y
    0x3E, 0x02, 0x04, 0x08, 0x10, 0x20, 0x3E, 0x00,  // 0x7A z
    0x1C, 0x10, 0x10, 0x10, 0x10, 0x10, 0x1C, 0x00,  // 0x7B {
    0x00, 0x20, 0x10, 0x08, 0x04, 0x02, 0x00, 0x00,  // 0x7C |
    0x1C, 0x04, 0x04, 0x04, 0x04, 0x04, 0x1C, 0x00,  // 0x7D }
    0x08, 0x14, 0x22, 0x00, 0x00, 0x00, 0x00, 0x00,  // 0x7E ~
    0x3F, 0x3F, 0x3F, 0x3F, 0x3F, 0x3F, 0x3F, 0x00,  // 0x7F 0x7F
};

// Helper: get pattern byte for a character code and row
inline uint8_t TRS80_GetCharPattern(uint8_t char_code, uint8_t row) {
    if (char_code >= TRS80_CHAR_COUNT || row >= TRS80_CHAR_BYTES_PER_CHAR) {
        return 0x00;
    }
    return TRS80_CHAR_GEN[char_code * TRS80_CHAR_BYTES_PER_CHAR + row];
}
